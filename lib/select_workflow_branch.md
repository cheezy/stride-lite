...
**This file is the only place a branch is decided.** Explore, plan and review have one gate each, and each gate is a cell above. Nothing else in this repository may supply a second condition that reaches the same outcome on its own — not the six-column restatement under Step 3 of `skills/stride-lite-workflow/SKILL.md`, not the quick reference card, not `README.md`, and not a task file's own prose. Those surfaces are free to describe an outcome; none of them may create one. Where any of them reads as an independent trigger, it is wrong and this table stands. The workflow skill already states the same arrangement from its end — its derived table yields to this helper whenever the two disagree — and this paragraph is the half that had been missing: the skill said which side loses, and nothing said that this side is the only side. D221 and D232 are the failure being kept out. Two separately reachable ways to arrive at one dispatch do not make the dispatch twice as certain; they make the recorded reason for it stop identifying anything.

<!-- canon:row-precedence v1 -->
**Reading order, and why nothing here is a tie-break — once the table is reached at all.** One condition is settled before any row is consulted: a task whose `## Key files` heading is missing or unrecognizable resolves to `full` outright, as the Input rules below spell out and the Invariants restate. That state is not a row and has no cell in the Key files column, so a reader who goes straight to the table will match the first row on complexity alone and arrive at `skip-all` — the one direction this file repeatedly names as the one that ships an unreviewed diff. Read the input rules first; the table governs what survives them.

**What the table does guarantee, for the inputs that reach it.** Scan down and take the first row that matches, and it will never have to arbitrate. Three complexity values are named outright — `small`, `medium`, `large` — with `small` split across two rows by a key-file count that is either at most one or at least two, no gap at the boundary and no overlap on it, and a final row reached exactly when the other four are not. No surviving input satisfies two rows, so the row a reader stops at is the only row it could have stopped at. That is a stronger property than precedence and worth naming, because a table whose rows overlap has to publish a ranking, and a ranking is one more thing that can be got wrong.

**What the order is still load-bearing for.** Two things, and only two. The `small` pair has to be read 0–1 before 2-or-more: a reader who takes the second of them as the general `small` case and the first as an exception will hand one-file work to `explore-review`, and stating the cheaper row first is what makes the threshold read in a single direction. And the last row has to stay last, because it fires on a complexity that could not be read at all — never on a contest between rows that both matched — and lifting it would let it start answering for values the three named rows already answer for. If a row is ever added that can overlap an existing one, this paragraph stops being a description of the table and has to become a ranking instead; add the row and the ranking in the same change, or add neither.
