...
- **No hard dependency on another plugin.** The exploratory-testing integration is optional in both directions: every gate is checked at runtime against this session's available surfaces, a closed gate is a clean skip, and nothing in `plugin.json` or any skill requires `stride-exploratory-testing` to be installed. Do not add a required dependency, and do not execute another plugin's content to probe for its presence — check availability only.
