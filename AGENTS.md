...
- **`skills/<name>/SKILL.md` files orchestrate.** They wire `lib/` helpers and the `agents/` subagents together — the create flows dispatch `create-decomposer`; the workflow skill dispatches `task-enricher`, `task-explorer` and `task-reviewer`. They never duplicate logic that lives in `lib/`.
