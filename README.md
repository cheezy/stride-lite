...
6. **Dispatch `stride-lite:task-reviewer`** against the task file with the working-tree diff (appends `## Review Report`) — **gated by the same matrix row resolved at step 3**
   - **Step 6a — manual & exploratory testing** — optional and gated: runs only when the task lists manual tests, the `stride-exploratory-testing` plugin is available, and the user gave an authorized-and-non-production affirmative at step 0. Maps each manual test to a charter and dispatches `stride-exploratory-testing:explorer` once per charter, with an explicit session budget. Any gate closed is a clean skip that fails nothing
   - **Step 6b — harden findings into regression checks** — optional and gated on step 6a having returned convertible findings and `/stride-exploratory-testing:harden` being available. Drafts land staged under `.exploratory/checks/`, outside the test tree; a draft is never reported as passing, because `/harden` holds no test runner
   - **Step 6c — deep security-considerations review** — optional and gated on the task listing at least one real security consideration (the `- (none)` placeholder does not count), the `stride-security-review` plugin being available, and this session being able to dispatch its agent. Dispatches `stride-security-review:security-reviewer` in explicitly-declared considerations mode and captures one verdict per consideration. Unlike steps 6a and 6b it runs on **every** review iteration, and any `partial` or `unmitigated` verdict loops back to implementation under the existing `max_review_iterations` cap rather than adding a new one
7. **Review-loop decision** — parse the review's status: `approved` → continue; `changes_requested` → loop back to step 4, under a ceiling of **two rounds** (`max_review_iterations`, clamped at two; a lower value is honoured). Reaching the ceiling is not one unconditional stop: remaining `important` and `minor` findings are recorded in the Completion Summary and the task completes, while a `category: "security"` finding or an outstanding 6a/6c escalation still stops the drive with the review surfaced. A `critical` gets exactly one further round scoped to it and then stops if it still stands — one extra round for the whole task, never one per pass. A round whose findings are every one of them `cosmetic` buys no further round, but only when each entry is a real-boolean `cosmetic` on a non-security `minor`; any other entry voids that branch and the round loops
