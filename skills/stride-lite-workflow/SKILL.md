...
**Two review rounds is the ceiling, and the second verifies rather than re-opens.** A round is a reviewer dispatch that left a `## Review Report` this step could read. A dispatch that errored, or came back without writing the section at all, is re-dispatched **once** and costs no round — a crash is not a finding, and the number of rounds and the number of times the agent was called are deliberately different numbers. A second consecutive failure is a dispatch error and stops the drive, as it always has. On round two, name in the dispatch prompt which findings the Step 4 pass addressed and pass `review_round: 2`; the reviewer still reads the whole diff and still renders every subsection, because the round scopes its mission and never its evidence.

**Apply the clamp first — this is a step, not a description.** Before any comparison below, set `max_review_iterations` to `min(max_review_iterations, 2)`, and treat an absent, non-integer or below-1 value as `2`. The inputs table describes this ceiling; this sentence is what performs it, and without a performing step a caller could raise the very bound that is supposed to make the loop terminate by construction. `review_iteration` starts at `0` on first entry to this step for the active task — it is scoped to one task and is never carried across tasks.

- If `status == "approved"` → **first confirm the report is conforming: `issues[]` must be empty.** A conforming reviewer never pairs an approval with findings — `agents/task-reviewer.md` defines `approved` as an empty `issues[]`, `minor` included — so a non-empty one is a contract violation, not a mild one. **Route it into the `changes_requested` branch below** — increment `review_iteration` and loop, exactly as an ordinary refusal does, so it consumes a round and is bounded by the ceiling like any other. A path that looped without incrementing would be the one place this loop does not terminate by construction, which would defeat the clamp above. This is the approved path's counterpart to the all-cosmetic re-check below, and it exists for the same reason: without it a `minor` carrying `category: "security"` could ride an approval straight into Step 8, where neither the ceiling carve-outs nor the all-cosmetic re-check runs, and be recorded rather than fixed. Once `issues[]` is empty this cannot arise, which is what makes Step 8's guarantee true on every path rather than only on two. → proceed to Step 8.
- If `status` is neither `"approved"` nor `"changes_requested"` → treat it as `changes_requested`, on the same conservative reasoning as the JSON parse fallback below, and name the value you actually received in the Completion Summary. Never read an unrecognized status as an approval.
- If `status == "changes_requested"` → increment the `review_iteration` counter and:
  - If `review_iteration < max_review_iterations` → loop back to **Step 4** (Implementation). Make further code changes addressing the reviewer's issues. Then re-run Steps 5, 6, **6c** and 7 in sequence. **6c is in the ordinary re-run set** because it runs on every pass; 6a and 6b stay out under their at-most-once rule.
...
- **Remaining `important` and `minor` findings are recorded, not fixed.** Write the Completion Summary and list each one by severity, `category` and `file:line`, restated in **one line of your own words** — never pasted. **Never copy a credential, token, internal hostname or customer datum out of a finding**; substitute `[REDACTED — text embedded a credential]` and identify the finding by its `file:line`. That is the same discipline Step 8's exploratory and security bullets carry, stated here rather than delegated to a file-wide rule this skill does not have. The task completes. An unfixed finding written where the next reader will find it is worth more than a third pass that turns up a fourth thing.
- **A `critical` that still stands is exempt from the ceiling and always blocks — for exactly one further round.** Fix it and dispatch **one** round scoped to that finding. If a `critical` still stands after that round, or you cannot fix it, take the stop path — clear the marker, surface the finding, write no Completion Summary. **The exemption is spent once and does not renew**: it is one extra round for the whole task, not one per pass, and a second `critical` arriving on the extra round does not buy a third. Bounding it is the point — an unbounded carve-out would be the retry budget the paragraph below promises no carve-out adds, and would hand back the ceiling the clamp above exists to guarantee. Never record a `critical` and complete.
- **A finding whose `category` is `"security"` is never merely recorded, at any severity.** Fix it, or take the same stop path. The reviewer's default severity for one is `important`, which is precisely the level the recording rule would otherwise sweep up — which is why it is stated separately rather than left to the bullet above.
- **An outstanding escalation takes the stop path too.** A Step 6a Critical this diff introduced, or a Step 6c consideration still `partial` or `unmitigated`, is never recorded-and-completed.

**A round whose findings are all cosmetic buys no further round.** Before incrementing on a `changes_requested`, read `issues[]`. If it parsed, is non-empty, and **every** entry both carries `cosmetic` as the real boolean `true` **and** is a `minor` whose `category` is not `"security"`, the loop has nothing left to converge on: do not increment, do not loop, go to Step 8 and carry those findings into the Completion Summary as recorded rather than fixed.

**Re-read `severity` and `category` here; do not take the flag's word for it.** The reviewer refuses `cosmetic: true` on anything but a non-security `minor`, but `agents/task-reviewer.md` states plainly that its refusals are producer-side and that nothing downstream re-checks them. **This step is that downstream, and this conjunct is the re-check.** One entry whose `severity` is anything other than `minor`, or whose `category` is `"security"`, voids the branch outright however it is flagged — the round increments and loops as normal. Without it a single mis-flagged entry would carry a `critical` or a security finding straight past the carve-outs above, which this branch never reaches because it fires *before* the increment. A `cosmetic` that is not a real boolean — `1`, `"true"`, `"yes"` — is not coerced here either: it fails the test, so the round loops.

Four further boundaries keep this from becoming an escape hatch. An absent or empty `issues[]` is never an all-cosmetic round — a `changes_requested` with nothing enumerated still loops. The prose fallback below cannot reach this branch at all: with no array to read the rule is **inapplicable**, not satisfied. Neither escalation branch is covered by it, because neither lives in `issues[]`. And a `not_met` project check whose subject is security takes the stop path whatever its `category` string reads and whatever its severity defaulted to — the carve-out above selects on a label the reviewer assigns itself, so judge such a finding by what it describes rather than by how it is filed. The flag's definition, and the three shapes it refuses, sit in `agents/task-reviewer.md`; the canon anchor is there beside the definition, not here beside this mirror of it.
...
**This is one cap, not two.** The ceiling is `max_review_iterations` and nothing else. None of the carve-outs above adds a counter, a retry budget or a terminal state — each routes to the stop this step has always had, the same one the session- and security-escalation branches take. What changed is the *ordinary* terminus: a loop that runs out with only `important` and `minor` outstanding now completes with them recorded, where it used to stop incomplete. The strict terminus is unchanged, and now names `critical` and `category: "security"` explicitly.

**The ceiling is prose here, and that is the whole of it.** The full plugin pins the round count in a file and re-checks it mechanically before a completion is accepted. This port has no submission to pre-check and no counter file: `review_iteration` lives in the drive's own working state for the length of one task. So the ceiling, the record disposition and both carve-outs are enforced by an agent reading this paragraph and nothing else. Disclosed here rather than left implied — the remedy is that the `## Review Report` stays on the task file as the audit trail, and a reader can count the rounds it took.

**On a re-dispatch, tell the reviewer which round it is.** Pass `review_round` and name the findings the previous Step 4 pass addressed, so round two verifies those rather than re-deriving the whole review from scratch.

**Everything Step 7 sets down about the ceiling, from the anchor above to this line, is this file's own wording and not this file's own rule.** `stride/docs/port-canon.md` holds it as entry `review-round-cap`, which is what that anchor comment marks. Say it in whatever words read best — every port says it differently, and that is expected. Deciding it should require something else is not a decision this repository makes on its own: the canon entry and the anchor each carry a version, both move inside the one change that alters the substance, and `stride/scripts/check-port-canon.sh` is what notices when only one of them did.
...
**Reaching Step 8 is a conjunction.** Proceed only when the `## Review Report` reads `approved` — **or** this step reached one of the two sanctioned non-approval termini above, the ceiling's record disposition or the all-cosmetic branch — **and** no Critical Step 6a attributed to this diff stands **and** every consideration Step 6c **returned a verdict for** came back `mitigated`. An `approved` report is necessary and no longer sufficient — necessary on the ordinary path, and on no path sufficient by itself. Those two termini are the only exceptions to the first half, and each carries its own conditions above rather than relaxing these.
...
- The review outcome, which depends on whether Step 6 ran:
  - **Review ran and approved** → a reference to the embedded review JSON's `status`, which reads `approved`.
  - **Review ran, did not approve, and Step 7 completed anyway** → the two sanctioned non-approval termini, the ceiling's record disposition and the all-cosmetic branch. Give the status the report actually carries — `changes_requested` — name which terminus was taken, and list the findings that were recorded rather than fixed. **Never write "approved" for a review that refused**, for the same reason the next bullet forbids it for a review that never happened: this file is the port's entire audit trail, there is no server holding a second copy, and a false approval in it is unrecoverable.
...
- **Any finding recorded rather than fixed** — whether at the review ceiling or on the all-cosmetic branch — by severity, `category` and `file:line`, restated in your own words. **Never copy a credential, token, internal hostname or customer datum out of a finding**; substitute `[REDACTED — text embedded a credential]` and identify it by its `file:line`, exactly as the security-considerations bullet above does. Omit this bullet only when the loop converged on an `approved` report with nothing outstanding. A `critical` or a `category: "security"` finding never appears here, on any path — Step 7 routes both to its stop path, on the ceiling branch by its carve-outs and on the all-cosmetic branch by the severity/category re-check that voids it, and a stop writes no Completion Summary for them to appear in. The approved path reaches the same guarantee from the other direction: an approval whose `issues[]` is non-empty is refused as non-conforming and loops, so there is no finding of any severity under an approval for this bullet to record.
...
**On the fleet `dispatch_count` key — it does not apply to this port, and here is the structural reason.** The port canon (`stride/docs/port-canon.md`, entry `dispatch-count-telemetry`) puts an optional integer `dispatch_count` on a `workflow_steps` entry with `dispatched: true`, counting dispatches rather than rounds, and binds it to the six reading limits its own file carries. Neither half has a host here. This plugin emits no `workflow_steps` object — the telemetry above is a bare array in a committed markdown file with no completion payload behind it — so there is no entry of that kind for the key to sit on, and no submission for the limits to travel with. More directly: the rule in the paragraph immediately above is the opposite design decision, taken deliberately. Ten fixed names, one entry each, `duration_ms` totalled across dispatches, because a reader counting entries here is counting steps and not attempts. A per-name attempt count restores the exact dimension that rule exists to collapse, in a schema no consumer parses and no endpoint validates. And the first of those six limits — that wall-clock is not token cost — already bites the totalling rule above, so importing the key without anywhere to state the limits would import the hazard without the warning.

**The canon already records this port as `not_applicable` on that entry, and this paragraph is the grounds its row cites.** No anchor is added: an anchor on a narrowed cell claims a compliance the port does not have, and the fleet drift check reports it as unexpected — which is the check working, not something to route around. Reopen the question if a `workflow_steps` payload and a completion endpoint ever land here. The objection is to a count with nothing to sit on and no limits travelling beside it, not to counting dispatches.
...
The gap is written down rather than closed. This port carries no canon anchor for that rule, and it should not. The canon's row for this port was corrected by **D302** to `not_applicable` on these same transport grounds, so the fleet drift check now reads that cell as narrowed-with-a-reason rather than missing, and an anchor sitting on it would register as an unexpected claim of compliance. If a completion API and a `workflow_steps` payload ever arrive in this port, reopen the question — the objection is to codes that cannot occur, not to the vocabulary.
...
- **task-reviewer agent dispatch fails, errors, or returns without writing a `## Review Report`** — re-dispatch it **once**; per Step 7's round definition that retry costs no review round, because a crash is not a finding. On a second consecutive failure, clear the marker, surface the reviewer's error and stop. Without a review verdict, the workflow can't decide Step 7.
...
- **Review-loop reaches max_review_iterations with a `critical`, a security finding — `category: "security"`, or any `not_met` project check whose subject is security whatever its label reads — or an outstanding 6a/6c escalation standing** — clear the marker and stop without writing the Completion Summary. (Reaching it with only `important` and `minor` outstanding is not a stop: Step 7 records those and completes.) The task file retains its latest `## Review Report` section as the audit trail. The user can manually fix the issues and re-run the workflow; on re-run the task is "incomplete" (no Completion Summary) so Step 1 picks it up again.
...
- **"The reviewer's `changes_requested` looks minor — I'll write the Completion Summary anyway."** No. Step 7 has exactly two sanctioned ways to complete without an approval, and neither is a judgement call about how a finding looks: the ceiling's record disposition, reached only after the rounds are actually spent, and the all-cosmetic branch, reached only when every entry passes the severity and category re-check. Outside those two, `approved` proceeds and anything else loops back. Deciding for yourself that a finding is not worth another round is the bypass this guards.
...
- **"The review-loop has hit the ceiling but the reviewer keeps finding the same issue — I'll force-approve."** No, and note the ceiling is two rounds, not three. What to do depends on what is outstanding, and neither answer is an approval: with only `important` and `minor` findings left, record them in the Completion Summary and complete; with a `critical`, a security finding, or an outstanding escalation, clear the marker, stop, surface the issue and let the user intervene. Writing `approved` over a report that says otherwise defeats the entire review-loop purpose.
...
- **Don't loop forever in Step 7.** The `max_review_iterations` ceiling (default 2, clamped at 2) is mandatory. On reaching it, apply Step 7's disposition: record remaining `important` and `minor` findings and complete; stop with the review surfaced for a `critical`, a security finding — judged by what it describes, not only by a `category` string the reviewer assigns itself — or an outstanding escalation.
...
├─ explorer dispatch errors, or a SECOND consecutive reviewer dispatch error
│  (the first reviewer failure is re-dispatched once and costs no round)
├─ goal dir malformed: no goal.md, no task1.md, or a numeric gap
├─ review ceiling reached with a critical, a security finding (judged by
│  subject, not only by the category label), or a standing escalation
└─ a command you need is not on the Bash scope allow-list
   ALL FIVE: clear the marker first, write no Completion Summary

WHAT COMPLETES WITHOUT AN APPROVAL — exactly two, both in Step 7
├─ the ceiling reached with only important/minor left: record them, complete
└─ an all-cosmetic round: every entry a real-boolean cosmetic, minor,
   non-security. Any other entry voids it and the round loops.
   NEITHER is available for a critical, a security finding, or a standing
   6a/6c escalation. Report the status as changes_requested, never approved.
