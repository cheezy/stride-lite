...
The default is now two, and a larger value is clamped to two rather than honoured; `1` is still honoured, because lowering a ceiling is always safe. A round is now defined as a reviewer dispatch that left a readable `## Review Report`, so a dispatch that errored or wrote nothing is re-dispatched once and costs no round — which also closes a gap where a reviewer that wrote nothing fell through Step 7's no-review branch and completed the task unreviewed. On round two the dispatch names the findings the last implementation pass addressed and passes `review_round: 2`; the reviewer still reads the whole diff and still renders every subsection.

Reaching the ceiling now splits four ways. Remaining `important` and `minor` findings are **recorded** in the Completion Summary by severity, category and `file:line` and the task completes — a softening of the ordinary path. A `critical` that still stands is exempt from the ceiling and always blocks, **for exactly one further round**, spent once for the whole task and never renewed; if it still stands after that round the drive stops. An unbounded exemption would have re-applied to its own post-state and handed back the very ceiling this change exists to establish — which would also have contradicted the "no carve-out adds a retry budget" paragraph two paragraphs below it. A finding whose `category` is `"security"` is never merely recorded at any severity, which matters because `important` is the reviewer's default for one and is exactly the level the recording rule would otherwise sweep up; a `not_met` project check whose subject is security takes the stop path whatever its `category` string reads, since that label is one the reviewer assigns itself. An outstanding Step 6a introduced Critical or a Step 6c `partial`/`unmitigated` consideration takes the stop path too.

**The clamp is performed by a step, not merely described by the inputs table.** Step 7 now applies `min(max_review_iterations, 2)` before any comparison and treats an absent, non-integer or below-1 value as `2`; `review_iteration`'s initialization is stated in Step 7 itself rather than cross-referenced to a step that never mentioned it. A constraint that lives only in a table cell does not constrain the procedure that reads the value.

**Step 8 gained a shape for a review that ran, refused, and completed anyway.** It previously offered only "review ran" — which asserted the status was `approved` "by contract, since Step 7 only reaches Step 8 on an approval" — and "review skipped". Both new termini falsify that premise, so an agent following the old bullet would have written `approved` into the audit trail of a task the reviewer refused, the same error the sibling bullet already forbade for a skipped review. The new shape reports the status the report actually carries, names which terminus was taken, and lists what was recorded. This file is the port's entire audit trail; there is no server holding a second copy.

**The crashed-dispatch retry is now stated in all three places that specify it.** The error-handling list and the quick-reference card both said a reviewer dispatch error stops the drive on the first failure, which the new round definition contradicts. Both now say the first failure is re-dispatched once at no cost in rounds and the second stops the drive. The card also names the two non-approval termini, which it previously omitted while claiming to list Step 7's exits.

**This adds no second cap and no new terminal state**, and the standing prohibition that says so is left in the file unedited. Each carve-out routes to the stop Step 7 has always had — the same one the session- and security-escalation branches take. What changed is the ordinary terminus, not the strict one.

**The ceiling is prose in this port, and the text says so rather than implying otherwise.** The full plugin pins the round count in a file and re-checks it mechanically before a completion is accepted; there is no submission here to pre-check and no counter file, so the ceiling, the record disposition and both carve-outs are enforced by an agent reading the paragraph. The stated limit is written into the skill rather than left for a reader to infer. `README.md`, the error-handling table, the pitfall list and the quick-reference card were all updated to the new terminus.

### Added — a cosmetic finding is a disposition, and an all-cosmetic round ends the loop (W2170)

An `issues[]` entry may now carry an optional boolean `cosmetic` (canon entry `cosmetic-finding-class`). Absent means `false`. It is a disposition and not a fourth severity: the entry keeps its place, its `severity` and its `category`, the report still lists it under Minor, and `status` is decided exactly as before. What it buys is downstream — a `changes_requested` round whose `issues[]` parsed, is non-empty, and carries `cosmetic: true` on **every** entry does not increment the counter and does not loop.

Three shapes are refused rather than weighed: `cosmetic: true` beside any severity other than `minor`; `cosmetic: true` beside `category: "security"` at any severity; and a value that is not a real boolean, since `1`, `"true"` and `"yes"` are not coerced. **The security exclusion ships in the same commit as the flag** — a port carrying the flag without it would be worse off than one carrying neither.

**The workflow re-reads `severity` and `category` rather than trusting the flag**, and that consumer-side conjunct is load-bearing rather than belt-and-braces. The all-cosmetic branch fires *before* the increment, so it never reaches the ceiling carve-outs at all — without the re-check, one entry mis-flagged `cosmetic: true` at `critical` or `category: "security"` would have completed the task on round one, past three of the port's own stated rules. The full plugin can rely on a mechanical pin refusing that shape at submission; this port has no submission, and its own text says the producer-side refusals are never result-verified. So the consumer performs the check: any entry that is not a real-boolean `cosmetic` on a non-security `minor` voids the branch and the round loops as normal.

Four further boundaries stop the rule becoming an escape hatch. An absent or empty `issues[]` is never an all-cosmetic round. The prose fallback cannot reach the branch at all — with no array to read the rule is inapplicable rather than satisfied. Neither escalation branch is covered, because neither lives in `issues[]`. And a security-subject project check takes the stop path however it is categorized.

`cosmetic` is documented **locally** in `agents/task-reviewer.md` rather than by citation into stride, joining `security_considerations` on the same reasoning the file already gives: a citation into another repository cannot be checked from this one. It earns that on a sharper test, because getting it wrong costs a whole review pass. The reviewer's cited `schema_version` moves from `"1.6"` to `"1.7"` at both sites, since 1.7 is where the key arrived upstream and a block carrying it while stamping 1.6 declares a version predating its own field; `test/smoke.sh`'s two schema pins moved with it.

**This port cannot check the three refusals at the producer**, and the agent file says so plainly — but it no longer says they are unchecked altogether, because after the consumer-side re-check landed that was no longer true. There is no submission step between the reviewer writing the block and the workflow reading it back, so the refusals are producer-side discipline, self-certified and never result-verified. Disclosed rather than implied — and the consumer-side re-check above is what stops that disclosure from being the whole story for the one consequence that mattered.

### Fixed — an approval can no longer carry findings, which closes the one path a security `minor` could take to a Completion Summary (W2170)

The top-level verdict rule added in this release originally defined `approved` as "no `critical` and no `important` issue, every acceptance criterion `met`, and no section verdict `failed`", then said `changes_requested` applied "in every other case — including one where the only findings are `minor`". Those two clauses contradict each other on a `minor`-only round, and the contradiction was not academic: under the first reading a `minor` carrying `category: "security"` produced an `approved` report, took Step 7's ordinary approved path — where neither the ceiling carve-outs nor the all-cosmetic re-check runs — and was written into the Completion Summary as recorded-not-fixed, past this same release's rule that a security finding is never merely recorded at any severity. The first reading also made the all-cosmetic branch unreachable, since it only ever fires on a `changes_requested`.

`approved` is now defined by an **empty** `issues[]`, of every severity with `minor` named explicitly, so the two clauses agree. Step 7's approved branch additionally refuses a non-conforming approval that carries findings and loops instead, which is the approved path's counterpart to the all-cosmetic re-check and rests on the same reasoning: the reviewer's own discipline is producer-side, and this port is the only place it can be re-checked. Step 8's guarantee that a `critical` or a security finding never appears in a Completion Summary now holds on every path rather than on two of the three.

### Fixed — three smoke assertions pinned their neighbours rather than the clauses they were added to protect (W2170)
...
Found while porting the same rules to `stride-copilot-lite`, which inherited the wording from here. Fixed in both before either was released.

### Added — the top-level review verdict vocabulary is documented in the contract that emits it (W2170)

`skills/stride-lite-workflow/SKILL.md` Step 7 branches on a top-level `status` of `approved` or `changes_requested` more than thirty times. Neither token appeared anywhere in `agents/task-reviewer.md`, which documented only the section-level verdicts (`passed` / `failed` / `not_assessed`) and the consideration-level ones (`mitigated` / `partial` / `unmitigated`) and covered the top-level field by citation into stride's reviewer. That citation is exactly what this file elsewhere says cannot be relied on — "a citation into another repository cannot be checked from this one" — and this port installs without that repository present, so the port's single review decision point was undecidable from the artifacts it ships. Worse, the values it *did* document collide: a reviewer following the contract literally was most likely to emit `failed`, a value Step 7 had no branch for.

The two values are now defined locally, with an explicit warning that none of the six section- or consideration-level values is ever legal at the top level, and a rule for which to emit. Step 7 additionally gained a conservative branch for an unrecognized status: treat it as `changes_requested` and name the value received, never as an approval.

### Added — the ported rules are pinned by the port's own test suite (W2170)

Both ported rules are prose in this port, so `test/smoke.sh` is the only mechanical bound available — and the port's established idiom is to pin load-bearing contract literals with `assert_has`. Assertions were added covering: the `cosmetic` definition and each of its three refusals; the security-category exclusion specifically, which the task's own security consideration names as the clause that must never ship apart from the flag; the two canon anchors, each pinned to exactly one occurrence; the absence of any `dispatch-count-telemetry` anchor in the shipped contract files; the ceiling default and the performing clamp step; each of the four ceiling dispositions including the bounded critical carve-out; the all-cosmetic branch's severity/category re-check and its boolean strictness; Step 8's non-approval shape and its prohibition on writing `approved` for a refusal; the redaction clause at both recorded-finding write sites; and the top-level verdict vocabulary. The suite passes clean with them in place.

Without these, an edit could have deleted the security exclusion, left the flag in place, and kept the suite green — producing precisely the flag-without-exclusion state the consideration calls worse than not porting at all.

### Not applicable — the fleet `dispatch_count` telemetry key has nothing to sit on in this port (W2170)

The canon's `dispatch-count-telemetry` entry puts an optional integer `dispatch_count` on a `workflow_steps` entry and binds it to the six reading limits in its own file. This port emits no `workflow_steps` object — its telemetry is a bare array in a committed markdown file with no completion payload behind it — so there is no entry of that kind for the key to sit on and no submission for the limits to travel with. More directly, the port's ten-fixed-names rule deliberately collapses attempts into a step and totals `duration_ms` across dispatches; a per-name attempt count would restore the exact dimension that rule exists to suppress.

The grounds are now written into `skills/stride-lite-workflow/SKILL.md` beside that rule, which is where the canon's row already said this port records them. **No anchor is added**, and that is the correct state rather than an omission: an anchor on a narrowed cell claims a compliance the port does not have and the fleet drift check reports it as unexpected. The verdict is *cannot-apply*, not *not-yet-applied*, and the two are distinguished structurally — by the canon row plus the absent anchor — rather than by adjective.

`stride/docs/port-canon.md` is not edited by this release. Rules `review-round-cap` and `cosmetic-finding-class` stay `required` and go green by this port gaining their anchors; `dispatch-count-telemetry` was already `not_applicable` and its cited grounds now resolve to a paragraph genuinely about `dispatch_count`.

### Fixed — stale cap references and two rationalization bullets the change falsified (W2170)

The skill's YAML front matter still summarised the loop as "cap: 3 iterations" — the description a reader and the loader see before any step body. Two entries in the rationalization list had become false rather than merely stale: one asserted "the Step 7 contract is binary: `approved` proceeds, anything else loops back", which both new termini contradict, and an agent reading it would have refused the very branches this release adds; the other prescribed the old unconditional stop for a loop that had "hit 3 iterations". All three now describe the two-round ceiling and its four-way disposition.

### Fixed — the `reason_code` non-adoption no longer asserts a canon row that has since been corrected (W2170)

The gap paragraph in `skills/stride-lite-workflow/SKILL.md` claimed the drift check "reports the cell missing — which is the accurate reading" and that the canon's row for this port "is the thing that is actually wrong". Both clauses were overtaken by **D302**, which corrected that row to `not_applicable` on the same transport grounds. A `not_applicable` cell is swept and reports its own informational line, not a missing one. The paragraph now says the row is right and that an anchor on it would register as an unexpected claim of compliance; its structural argument for non-adoption is unchanged.

The 0.14.0 entry below carries the same overtaken claim. It is left standing rather than rewritten — a dated entry records what a release said at the time, and this section is the changelog's own mechanism for superseding it. Its heading word `Deferred` is also the deferred/not-applicable conflation D302 identified, which is why the `dispatch_count` section above is headed `Not applicable` instead.
